...
int main(int argc, char *argv[])
{
  ...
  /*Create socket, bind and wait for Alice*/
    ...
    while(accepting){
        ...
        ReceiveMessageFromPeer(server.ssl);
        SendPeerAMessage((char*)"Hi Alice!", server.ssl);
        sleep(5);
        ReceiveMessageFromPeer(server.ssl);
        SendPeerAMessage((char*)"No, sorry!", server.ssl);
        sleep(5);
        ReceiveMessageFromPeer(server.ssl);
        SendPeerAMessage((char*)"Ciao!",server.ssl);
        accepting = 0;
    } //end while(accepting)
        close(client_sock);
  /*  }*//*end while (true)*/
